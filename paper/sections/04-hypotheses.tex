...
\emph{The NVLink exchange} (\cref{sec:hyp-exchange}): the composed gate
passed flag-only ($-14\%$) and failed with payload, because the
...
($-10\%$) and the registered comparison applies there: 4.92~\textmu s
against a bar of 10.78 (half the pinned NCCL floor at that size) ---
\textbf{confirmed at 4~KiB and below, margin 2.2$\times$}. At 20~KiB the
remediated composition still under-predicts by 28\%: about 2.3~\textmu s
of round trip is a first-read-after-peer-write visibility cost with no
table row yet, so the formal comparison stays blocked there, with the
raw 1.6$\times$ margin published as observational.
