...
cycle in its four-tile form --- 512 bytes per instruction, exactly the
64-byte-per-cycle unified-datapath ceiling of \cref{sec:memory}: feeding
the tensor cores is bandwidth-bound on the same path as every other
shared-memory access. \Cref{tab:tensor} summarises.
