...
data existed (repository commit \texttt{c6e1a34}, 2026-06-10, preceding
every measurement it governs). The commit history is the receipt. A
later message-only history rewrite renamed every commit while
preserving trees and dates byte-for-byte; the original history remains
on the \texttt{main-original} branch with an old-to-new map in the
repository, so the receipt and the provenance stamps in the raw data
both stay resolvable. One
