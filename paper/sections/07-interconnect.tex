...
\texttt{atomicAdd} chains at 541~ns and a peer CAS at 552--558~ns. The
...
loses rather than gains: the fabric, not issue width, binds. With one
GPU streaming inbound
...
2.59~\textmu s against 1.14~\textmu s for the same bytes at steady
state. This
visibility cost has its own table row, measured by timing only the
...
kernels in 3.90~\textmu s empty, 4.92~\textmu s at 4~KiB, and
7.94~\textmu s at 20~KiB, with
...
collects the rows, and \cref{fig:exchange} sets the primitive against the
...
registered comparison applies across its domain: margins 2.1$\times$ at
